...
import { PushSenderService } from '../push-sender/push-sender.service';
  ...
  implements OnModuleInit, OnApplicationShutdown
{
  ...
  constructor(private readonly pushSenderService: PushSenderService) {}
  ...
  async onModuleInit() {
    ...
    try {
      ...
      const routingKey = process.env.ROUTING_KEY || 'notification.push';
      const queue = process.env.QUEUE || 'notifications';
  ...
  private async handleMessage(msg: amqp.ConsumeMessage | null) {
    ...
    try {
      ...
      await this.pushSenderService.handlePushEvent(
        content,
